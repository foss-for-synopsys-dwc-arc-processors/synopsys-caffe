...
void PyramidRoiAlignLayer<Dtype>::LayerSetUp(
  ...
  axis_ = pyramid_roi_align_param.axis() < 0
              ? bottom[0]->num_axes() - 1 + pyramid_roi_align_param.axis()
              : pyramid_roi_align_param.axis();
  CHECK_GE(num_output_, 1) << "num_output must not be less than 1.";
  CHECK_GE(axis_, 0) << "axis must not be less than 0.";
  CHECK_LE(axis_, bottom[0]->num_axes() - 1)
  ...
  CHECK_GT(pyramid_roi_align_param.crop_h(), 0) << "crop_h must be > 0";
  CHECK_GT(pyramid_roi_align_param.crop_w(), 0) << "crop_w must be > 0";
...
void PyramidRoiAlignLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom,
  ...
  if (data_format_ == "NHWC") {
    ...
    CHECK_EQ(bottom[2]->shape(1), bottom[3]->shape(1))
        << "Input images should have equal channel count.";
    CHECK_EQ(bottom[2]->shape(1), bottom[4]->shape(1))
        << "Input images should have equal channel count.";
  ...
  indices_shape_out_.push_back(
      num_output_); // only 1 dimension for topk selection
...
template <typename Dtype>
void PyramidRoiAlignLayer<Dtype>::Crop_And_Resize(
    const Dtype *bottom_data, const Dtype *bottom_rois, Dtype *top_data,
    int num_boxes_, int image_height_, int image_width_, string data_format_) {
    ...
    if (data_format_ == "NHWC") {
      for (int y = 0; y < crop_height_; ++y) {
        ...
        if (in_y < 0 || in_y > image_height_ - 1) {
          for (int x = 0; x < crop_width_; ++x) {
            for (int d = 0; d < channels_; ++d) {
              top_data[((b * crop_height_ + y) * crop_width_ + x) * channels_ +
                       d] = extrapolation_value_;
        ...
        for (int x = 0; x < crop_width_; ++x) {
          ...
          for (int d = 0; d < channels_; ++d) {
            int index = ((b_in * image_height_ + top_y_index) * image_width_ +
                         left_x_index) *
                            channels_ +
                        d;
            ...
            index = ((b_in * image_height_ + top_y_index) * image_width_ +
                     right_x_index) *
                        channels_ +
                    d;
            ...
            index = ((b_in * image_height_ + bottom_y_index) * image_width_ +
                     left_x_index) *
                        channels_ +
                    d;
      ...
      for (int d = 0; d < channels_; ++d) {
        for (int y = 0; y < crop_height_; ++y) {
          ...
          if (in_y < 0 || in_y > image_height_ - 1) {
            for (int x = 0; x < crop_width_; ++x) {
              top_data[((b * channels_ + d) * crop_height_ + y) * crop_width_ +
          ...
          for (int x = 0; x < crop_width_; ++x) {
            ...
            int index = ((b_in * channels_ + d) * image_height_ + top_y_index) *
                            image_width_ +
                        left_x_index;
            //(image(b_in, d, top_y_index, left_x_index));
            const float top_left = static_cast<float>(bottom_data[index]);

            index = ((b_in * channels_ + d) * image_height_ + top_y_index) *
                        image_width_ +
                    right_x_index;
            //(image(b_in, d, top_y_index, right_x_index));
            const float top_right = static_cast<float>(bottom_data[index]);

            index = ((b_in * channels_ + d) * image_height_ + bottom_y_index) *
                        image_width_ +
                    left_x_index;
...
void PyramidRoiAlignLayer<Dtype>::Forward_cpu(
  ...
  const Dtype *image_meta = bottom[1]->cpu_data(); // only support batch_size =
                                                   // 1
  const int height_ = image_meta[4];
  const int width_ = image_meta[5];

  // calculate indices_data from bottom[0](boxes)
  const int last_shape = bottom[0]->shape(bottom[0]->num_axes() - 1);
  CHECK_EQ(4, last_shape) << "bottom[0].shape[-1] should equal to 4!!!";
  const int boxes_count = bottom[0]->count() / last_shape;
  const int image_area = height_ * width_;
  const float alpha1 = 224.0 / sqrt(image_area);

  vector<float> indices_data(boxes_count, 0);
  for (int i = 0; i < boxes_count; ++i) {
    indices_data[i] = (bottom_data_g[i * 4 + 2] - bottom_data_g[i * 4]) *
                      (bottom_data_g[i * 4 + 3] - bottom_data_g[i * 4 + 1]);
    indices_data[i] = sqrt(indices_data[i]) / alpha1;
    indices_data[i] = indices_data[i] > 0.0001 ? indices_data[i] : 0.0001;
    indices_data[i] = std::log2(indices_data[i]);
    indices_data[i] = std::rint(indices_data[i]) + 4;
    indices_data[i] = indices_data[i] > 2 ? indices_data[i] : 2;
    indices_data[i] = indices_data[i] < 5 ? indices_data[i] : 5;
  }
    ...
    if (axis_ == 1) {
      for (int i = 0; i < bottom[0]->shape(0); i++) {
        for (int j = 0; j < bottom[0]->shape(1); j++) {
          int value = int(indices_data[i * bottom[0]->shape(1) + j]);
          switch (value) {
          case 2:
            group.push_back(j);
            break;
          ...
          case 5:
            ...
            break;
          default:
            LOG(FATAL) << "The value " << value
                       << " is out of condition range [2, 3, 4, 5]!";
            break;
          }
        }
        ...
        for (int j = 0; j < group1.size() && (j + group.size()) < num_output_;
             j++) {
          indices_.push_back(i);
          indices_.push_back(group1[j]);
          topk_indices_.push_back(group1[j]);
        }
        for (int j = 0; j < group2.size() &&
                        (j + group.size() + group1.size()) < num_output_;
             j++) {
          indices_.push_back(i);
          indices_.push_back(group2[j]);
          topk_indices_.push_back(group2[j]);
        }
        for (int j = 0; j < group3.size() && (j + group.size() + group1.size() +
                                              group2.size()) < num_output_;
             j++) {
          indices_.push_back(i);
  ...
  for (int m = 0; m < indices_.size() / indices_N_; ++m) {
    ...
    caffe_copy(gather_nd_size_, bottom_data_g + bottom_offset,
               top_data_g + top_offset);
  }
  ...
  Crop_And_Resize(bottom_data, bottom_rois, top_data, num_boxes_, image_height_,
                  image_width_, data_format_);
  ...
  Crop_And_Resize(bottom_data1, bottom_rois, top_data, num_boxes_,
                  image_height_, image_width_, data_format_);
  ...
  Crop_And_Resize(bottom_data2, bottom_rois, top_data, num_boxes_,
                  image_height_, image_width_, data_format_);
  ...
  Crop_And_Resize(bottom_data3, bottom_rois, top_data, num_boxes_,
                  image_height_, image_width_, data_format_);

  //<--topk reverse sort to get final indices
  std::vector<std::pair<Dtype, int>> bottom_data_vector(num_output_);
  for (int i = 0; i < num_output_; ++i) {
    bottom_data_vector[i] = std::make_pair(topk_indices_[i], i);
  }
  std::partial_sort(
  ...
  for (int m = 0; m < num_gather_; ++m) {
    for (int n = 0; n < num_output_; ++n) {
      ...
      caffe_copy(gather_size_, bottom_data_c + bottom_offset_out,
                 top_data_out + top_offset_out);
      num_c += 1;
    }
  }
  // final gather-->
