...
namespace caffe {
	...
	void ReduceProdLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		...
		const ReduceProdParameter& reduce_prod_param = this->layer_param_.reduce_prod_param();
		reduce_prod_keepdims_ = reduce_prod_param.keepdims();
		reduce_prod_axis_.clear();
		std::copy(reduce_prod_param.axis().begin(),
			reduce_prod_param.axis().end(),
	...
	template <typename Dtype>
	void ReduceProdLayer<Dtype>::InReduceProd(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top,
		...
		for (int i = 0; i < shape_in[lv_in]; ++i) {
			int b_idx_add = i * bottom[0]->count(reduce_prod_axis_[lv_in] + 1);
			if (lv_in == shape_in.size() - 1) {
				top_data[t_idx] *= bottom_data[b_idx + b_idx_add];
			}
			if (lv_in < shape_in.size() - 1) {
				InReduceProd(bottom, top, b_idx + b_idx_add, lv_in + 1, t_idx);
			}
		}
	};

	template <typename Dtype>
	void ReduceProdLayer<Dtype>::OutReduceProd(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top,
		...
		for (int i = axis_dim_ - 1; i > -1; --i) {
			...
			idx_out.erase(idx_out.begin() + reduce_prod_axis_[i]);
		}
		// main part 
		for (int i = 0; i < shape_out[lv_out]; ++i) {
			...
			if (lv_out == shape_out.size() - 1) {				
				InReduceProd(bottom, top, b_idx + b_idx_add, lv_in, t_idx + t_idx_add);
			}
			if (lv_out < shape_out.size() - 1) {
				OutReduceProd(bottom, top, lv_out + 1, b_idx + b_idx_add, lv_in, t_idx + t_idx_add);
			}
		}
	};
    ...
    caffe_set(top[0]->count(), Dtype(1), top_data);
		if (axis_dim_ == 0 || axis_dim_ == bottom[0]->num_axes()) {
			...
			int b_idx = 0;
			int t_idx = 0;
			OutReduceProd(bottom, top, lv_out, b_idx, lv_in, t_idx);
		}
	}
