...
void MulLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  is_scalar_=false;
  Blob<Dtype>* multiplier = (bottom.size() > 1) ? bottom[1] : this->blobs_[0].get();

  if(bottom[0]->num_axes()==0 || multiplier->num_axes()==0)
    is_scalar_=true;
  dim_diff_ = bottom[0]->num_axes() - multiplier->num_axes();
  dim_ = bottom[0]->num_axes() >= multiplier->num_axes() ? bottom[0]->num_axes() : multiplier->num_axes();
  ...
  if(dim_diff_ == 0)
  {
    if(!is_scalar_)
    {
      ...
      {
        CHECK(bottom[0]->shape(i)==multiplier->shape(i) || bottom[0]->shape(i)==1 || multiplier->shape(i)==1)
              << "Dimensions must be equal or 1 in the bottoms!";
        top_shape[i] = bottom[0]->shape(i) >= multiplier->shape(i) ? bottom[0]->shape(i): multiplier->shape(i);
      }
    }
  }
  else if(dim_diff_ > 0) //bottom0 has more axes than bottom1
  {
    if(!is_scalar_)
    {
      ...
      for(int i=dim_diff_; i<dim_; i++)
        top_shape[i] = bottom[0]->shape(i) >= multiplier->shape(i-dim_diff_) ? bottom[0]->shape(i): multiplier->shape(i-dim_diff_);
    }
  ...
  else //dim_diff_<0, bottom1 has more axes than bottom0
  {
    if(!is_scalar_)
    {
      ...
      for(int i=-dim_diff_; i<dim_; i++)
        top_shape[i] = bottom[0]->shape(i+dim_diff_) >= multiplier->shape(i) ? bottom[0]->shape(i+dim_diff_): multiplier->shape(i);
    }
