...
void MaskrcnnDetectionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  bbox_std_dev_.clear();
  std::copy(maskrcnn_detection_param.bbox_std_dev().begin(), maskrcnn_detection_param.bbox_std_dev().end(),
  ...
  detection_min_confidence_ = maskrcnn_detection_param.detection_min_confidence();
  ...
  CHECK_EQ(bottom[0]->num_axes(), 3) << "bottom[0] corresponds to rpn_rois, which has dimension: 3";
  CHECK_EQ(bottom[1]->num_axes(), 3) << "bottom[1] corresponds to mrcnn_class, which has dimension: 3";
  CHECK_EQ(bottom[2]->num_axes(), 4) << "bottom[2] corresponds to mrcnn_bbox, which has dimension: 4";
  CHECK_EQ(bottom[3]->num_axes(), 2) << "bottom[3] corresponds to input_image_meta, which has dimension: 2";
  CHECK_EQ(bottom[0]->shape(2), 4) << "bottom[0] should have shape: [batch_size, POST_NMS_ROIS_INFERENCE, 4]";
  CHECK_EQ(bottom[1]->shape(2), bottom[2]->shape(2)) << "bottom[1] and bottom[2] should have same shape in axis 2";
  CHECK_EQ(bottom[2]->shape(3), 4) << "bottom[2] should have shape 4 in axis 3";
  CHECK_EQ(bottom[0]->shape(1), bottom[1]->shape(1)) << "bottom[0] and bottom[1] should have same shape in axis 1";
  CHECK_EQ(bottom[1]->shape(1), bottom[2]->shape(1)) << "bottom[1] and bottom[2] should have same shape in axis 1";
...
template <typename Dtype>
inline vector<int> MaskrcnnDetectionLayer<Dtype>::tf_nms(vector<vector<Dtype> > &pred_boxes, vector<int> &order, int topk, float iou_threshold) const
{
	...
	while (order.size() > 0) {
		...
		for (int i = 0; i < order.size() - 1; ++i) {
			Dtype x1 = std::max<Dtype>(pred_boxes[idx][0], pred_boxes[order[i+1]][0]);
			Dtype x2 = std::min<Dtype>(pred_boxes[idx][2], pred_boxes[order[i + 1]][2]);
			Dtype y1 = std::max<Dtype>(pred_boxes[idx][1], pred_boxes[order[i + 1]][1]);
			Dtype y2 = std::min<Dtype>(pred_boxes[idx][3], pred_boxes[order[i + 1]][3]);
			Dtype w = std::max<Dtype>(Dtype(0), (x2 - x1));
			Dtype h = std::max<Dtype>(Dtype(0), (y2 - y1));
			inter[i] = w*h;
			area_sum[i] = ((pred_boxes[idx][2] - pred_boxes[idx][0])*(pred_boxes[idx][3] - pred_boxes[idx][1])) + ((pred_boxes[order[i + 1]][2] - pred_boxes[order[i + 1]][0])*(pred_boxes[order[i + 1]][3] - pred_boxes[order[i + 1]][1]));
		}

		vector<int> left;
		for (int j = 0; j < order.size() - 1; ++j) {
			auto ratio = (area_sum[j] == 0) ? 0 : (inter[j] / (area_sum[j] - inter[j]));
...
void MaskrcnnDetectionLayer<Dtype>::Forward_cpu(
    ...
    //get width and height from image_meta, which should be the same for all batch data
	...
	for (int b = 0; b < batch_size_; ++b) {
		...
		for (int i = 0; i < N_; ++i) {
			...
			for (int p = 0; p < num_class_; ++p) {
				part_class_data[p] = class_data[b*N_*num_class_ + i*num_class_ + p];
			}
			
			class_scores[i] = *(std::max_element(part_class_data.begin(), part_class_data.end()));
			class_ids[i] = std::distance(part_class_data.begin(), std::max_element(part_class_data.begin(), part_class_data.end()));

			// bbox_data[b][i][class_ids[i]][0] ~ bbox_data[b][i][class_ids[i]][3]
			// delta_specific = delta_specific*bbox_std_dev_
			int b2 = b*bottom[2]->count(1) + i*bottom[2]->count(2) + class_ids[i] * bottom[2]->count(3);
			...
			auto center_y = rois_data[b3] + (0.5 + delta_specific[i*4])*(rois_data[b3 + 2] - rois_data[b3]);
			auto center_x = rois_data[b3 + 1] + (0.5 + delta_specific[i*4 + 1])*(rois_data[b3 + 3] - rois_data[b3 + 1]);
			auto height = (rois_data[b3 + 2] - rois_data[b3]) * std::exp(delta_specific[i*4 + 2]);
			auto width = (rois_data[b3 + 3] - rois_data[b3 + 1])* std::exp(delta_specific[i*4 + 3]);
			...
			refined_rois[i * 4] = std::max(std::min(refined_rois[i * 4], window[2]), window[0]);
			refined_rois[i * 4 + 1] = std::max(std::min(refined_rois[i * 4 + 1], window[3]), window[1]);
			refined_rois[i * 4 + 2] = std::max(std::min(refined_rois[i * 4 + 2], window[2]), window[0]);
			refined_rois[i * 4 + 3] = std::max(std::min(refined_rois[i * 4 + 3], window[3]), window[1]);
		...
		for (int k = 0; k < keep.size(); ++k) {
			...
			if (std::find(unique_pre_nms_class_ids.begin(), unique_pre_nms_class_ids.end(), pre_nms_class_ids[k]) == unique_pre_nms_class_ids.end()) {
				unique_pre_nms_class_ids.push_back(pre_nms_class_ids[k]);
			}
		}

		vector<int> nms_keep(unique_pre_nms_class_ids.size()*detection_max_instances_, -1);
		for (int d = 0; d < unique_pre_nms_class_ids.size(); ++d) {
			...
			vector<vector<Dtype> > nms_boxes(ixs.size(), vector<Dtype>(4)); //copy boxes value, shape=[ixs.size(), 4]
			...
			std::sort(nms_order.begin(), nms_order.end(), [&](int i, int j) {return nms_scores[i]>nms_scores[j]; });
			// get nms output
			vector<int> class_keep = tf_nms(nms_boxes, nms_order, detection_max_instances_, detection_nms_threshold_);
		...
		int num_keep = (keep2.size() > detection_max_instances_) ? detection_max_instances_ : keep2.size();
		//get tf.nn.top_k output
		int x = 0;
		std::iota(top_ids.begin(), top_ids.end(), x++); //Initializing
		std::sort(top_ids.begin(), top_ids.end(), [&](int i, int j) {return class_scores_keep[i]>class_scores_keep[j]; });
		vector<int> keep3;
