...
void ReduceMaxLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype> *> &bottom,
  ...
  const ReduceMaxParameter &reduce_max_param =
      this->layer_param_.reduce_max_param();
  reduce_max_keepdims_ = reduce_max_param.keepdims();
  reduce_max_axis_.clear();
  std::copy(reduce_max_param.axis().begin(), reduce_max_param.axis().end(),
...
template <typename Dtype>
inline int ReduceMaxLayer<Dtype>::offset(const vector<Blob<Dtype> *> &bottom,
  ...
  if (axis_dim_ == 0 || axis_dim_ == bottom[0]->num_axes()) {
    ...
    for (int i = 0; i < top_count; ++i) {
      ...
      for (int j = 0; j < bottom_count / top_count; ++j) {
        ...
        int offset_in = offset(bottom, reduce_max_axis_, ind_in);
