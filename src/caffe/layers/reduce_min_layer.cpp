...
namespace caffe {
	...
	void ReduceMinLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		...
		const ReduceMinParameter& reduce_min_param = this->layer_param_.reduce_min_param();
		reduce_min_keepdims_ = reduce_min_param.keepdims();
		reduce_min_axis_.clear();
		std::copy(reduce_min_param.axis().begin(),
			reduce_min_param.axis().end(),
	...
	template <typename Dtype>
	void ReduceMinLayer<Dtype>::InReduceMin(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top,
		...
		for (int i = 0; i < shape_in[lv_in]; ++i) {
			int b_idx_add = i * bottom[0]->count(reduce_min_axis_[lv_in] + 1);
			...
			if (lv_in < shape_in.size() - 1) {
				InReduceMin(bottom, top, b_idx + b_idx_add, lv_in + 1, t_idx);
			}
		}
	};

	template <typename Dtype>
	void ReduceMinLayer<Dtype>::OutReduceMin(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top,
		...
		for (int i = axis_dim_ - 1; i > -1; --i) {
			...
			idx_out.erase(idx_out.begin() + reduce_min_axis_[i]);
		}
		// main part 
		for (int i = 0; i < shape_out[lv_out]; ++i) {
			...
			if (lv_out == shape_out.size() - 1) {
				InReduceMin(bottom, top, b_idx + b_idx_add, lv_in, t_idx + t_idx_add);
			}
			if (lv_out < shape_out.size() - 1) {
				OutReduceMin(bottom, top, lv_out + 1, b_idx + b_idx_add, lv_in, t_idx + t_idx_add);
			}
		}
	};
	...
	void ReduceMinLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		...
		if (axis_dim_ == 0 || axis_dim_ == bottom[0]->num_axes()) {
			...
			int b_idx = 0;
			int t_idx = 0;
			OutReduceMin(bottom, top, lv_out, b_idx, lv_in, t_idx);
		}
	}
