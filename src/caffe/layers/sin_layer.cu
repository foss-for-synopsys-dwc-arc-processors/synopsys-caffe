...
namespace caffe {
  ...
  void SinLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    SinForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(count, bottom_data, top_data);
  ...
  void SinLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    ...
    if (propagate_down[0]) {
      ...
      const int count = bottom[0]->count();
      // NOLINT_NEXT_LINE(whitespace/operators)
      SinBackward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(count, top_diff, bottom_data, bottom_diff);
      CUDA_POST_KERNEL_CHECK;
    }
  }
