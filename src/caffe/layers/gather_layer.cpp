...
void GatherLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (indices_shape_.size() == 1 && indices_shape_[0] == 0) {         // indices_ is scalar/others, check whether shape matches value_size 
 	  indices_dim_ = 0;
  ...
  top_shape.resize(bottom_shape.size() + indices_dim_ - 1);     // resize the top_shape as r + q -1
