...
namespace caffe {
	...
	void MaskRCNNProposalLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype> *> &bottom,
	const vector<Blob<Dtype> *> &top) {
		...
		std::copy(this->layer_param_.maskrcnn_proposal_param().rpn_bbox_std_dev().begin(), this->layer_param_.maskrcnn_proposal_param().rpn_bbox_std_dev().end(),
		std::back_inserter(rpn_bbox_std_dev));
		
		batch_size = this->layer_param_.maskrcnn_proposal_param().batch_size();
		images_per_gpu = this->layer_param_.maskrcnn_proposal_param().images_per_gpu();
		//vector<float> RPN_BBOX_STD_DEV = this->layer_param_.maskrcnn_proposal_param().rpn_bbox_std_dev();
		pre_nms_limit = this->layer_param_.maskrcnn_proposal_param().pre_nms_limit();
		rpn_nms_threshold= this->layer_param_.maskrcnn_proposal_param().rpn_nms_threshold();
		post_nms_rois_inference = this->layer_param_.maskrcnn_proposal_param().post_nms_rois_inference();	  
...
//      vector<int> top_shape = { batch_size,pre_nms_limit,4};
	...
	template <typename Dtype>
	vector<vector<Dtype>> MaskRCNNProposalLayer<Dtype>::apply_box_deltas_graph(vector<vector<Dtype>> boxes,
	...
	template <typename Dtype>
	vector<vector<Dtype>> MaskRCNNProposalLayer<Dtype>::clip_boxes_graph(vector<vector<Dtype>> boxes,
	vector<Dtype> window){
		int num = boxes.size();		
	...
	template<typename T> 
	vector<int> argsort(vector<T>& array){
	...
	template <typename Dtype>
	vector<int> MaskRCNNProposalLayer<Dtype>::image_nms(vector<vector<Dtype>>& pred_boxes, vector<int>& order, int topk, float iou_threshold){
	...
	template <typename Dtype>
	Dtype* MaskRCNNProposalLayer<Dtype>::maskrcnnproposal(const Dtype* rpn_class,const Dtype* rpn_bbox,const Dtype* anchors){
		Dtype* rpn_rois =  (Dtype*) malloc(batch_size*post_nms_rois_inference*4*sizeof(Dtype));
		...
		for (int b = 0; b < batch_size; b++){
			...
			vector<vector<Dtype>> boxes = apply_box_deltas_graph (pre_nms_anchors_,deltas_);	
			...
			vector<int> indices = image_nms(boxes_, nms_order, proposal_count, nms_threshold);
			
//			vector<vector<Dtype>> proposals(proposal_count , vector<Dtype>(boxes_[0].size(),0.0));
			for(int i=0;i<indices.size();i++){
				for(int j=0;j<boxes_[0].size();j++){
					int idx = b*indices.size()*boxes_[0].size()+i*boxes_[0].size()+j;
	...
	template <typename Dtype>
	void MaskRCNNProposalLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
	const vector<Blob<Dtype>*>& top) {
