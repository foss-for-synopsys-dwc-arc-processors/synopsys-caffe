...
#include "caffe/layers/reduce_any_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {
template <typename Dtype>
void ReduceAnyLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype> *> &bottom,
                                       const vector<Blob<Dtype> *> &top) {
  const ReduceAnyParameter &reduce_any_param =
      this->layer_param_.reduce_any_param();
  keepdims_ = reduce_any_param.keepdims();
  axis_.clear();
  std::copy(reduce_any_param.axis().begin(), reduce_any_param.axis().end(),
...
template <typename Dtype>
void ReduceAnyLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom,
                                    const vector<Blob<Dtype> *> &top) {
...
template <typename Dtype>
inline vector<int>
ReduceAnyLayer<Dtype>::indices(int offset, const vector<int> &shape) const {
...
template <typename Dtype>
inline int ReduceAnyLayer<Dtype>::offset(const vector<Blob<Dtype> *> &bottom,
...
template <typename Dtype>
void ReduceAnyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
                                        const vector<Blob<Dtype> *> &top) {
...
INSTANTIATE_CLASS(ReduceAnyLayer);
REGISTER_LAYER_CLASS(ReduceAny);
