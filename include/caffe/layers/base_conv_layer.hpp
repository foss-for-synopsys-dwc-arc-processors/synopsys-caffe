...
class BaseConvolutionLayer : public Layer<Dtype> {
 public:
  ...
  template <typename LayerParam> void LayerSetUpInternal(LayerParam conv_param, const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top); //To distinguish SqueezeConv
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline int MinBottomBlobs() const { return 1; }
  virtual inline int MinTopBlobs() const { return 1; }
  virtual inline bool EqualNumBottomTopBlobs() const { return false; } //CUSTOMIZATION to false
  ...
  inline void conv_im2col_cpu(const Dtype* data, Dtype* col_buff) {
    if (!force_nd_im2col_ && num_spatial_axes_ == 2) {
      ...
      im2col_nd_cpu(data, num_spatial_axes_, conv_input_shape_.cpu_data(),
          ...
          pad_.cpu_data(), stride_.cpu_data(), pad_type_, dilation_.cpu_data(), col_buff); //CUSTOMIZATION
    }
  }
  inline void conv_col2im_cpu(const Dtype* col_buff, Dtype* data) {
    if (!force_nd_im2col_ && num_spatial_axes_ == 2) {
      ...
      col2im_nd_cpu(col_buff, num_spatial_axes_, conv_input_shape_.cpu_data(),
          col_buffer_shape_.data(), kernel_shape_.cpu_data(),
          pad_.cpu_data(), stride_.cpu_data(), pad_type_, dilation_.cpu_data(), data); //CUSTOMIZATION
    }
  }
#ifndef CPU_ONLY
